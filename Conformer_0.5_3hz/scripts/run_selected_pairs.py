...
# Run selected joint pairs through FINE (base), Conformer A and Conformer B on
# the FineMI 0.5-3 Hz data, then build the architecture x pair comparison table.
#
# Run from Conformer_0.5_3hz/ as a module (puts this folder on sys.path so
# `import src.*` resolves):
#
#   cd "FineMI-enhancements/Conformer_0.5_3hz"
...
def main():
    ...
    p.add_argument('--mode', choices=list(MODE_OVERRIDES), default='full',
                   help="smoke = 1 subject/1 split/2 epochs/800ms; sanity = 3 subjects/10 epochs; "
