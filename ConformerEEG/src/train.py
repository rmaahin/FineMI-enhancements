"""train_pair() — the notebook's within-subject training + testing loop (SPEC §8).

Ported from FineMI-enhancements/EMBC_deterministic-3.ipynb. Changes vs. the
notebook are marked inline:
  - model built by dispatching on cfg.model
  - execution-mode hooks (§11.3): subject cap, cfg.n_folds (n_folds == 1 ->
    single 60/20/20 split), cfg.n_epochs, cfg.test_time_windows_ms
  - best checkpoint saved with copy.deepcopy (the notebook's
    state_dict().copy() was shallow, so it silently tested last-epoch weights)
...
from src.data import (load_all_subjects, filter_binary, slice_time_windows,
...
def train_pair(class_a: int, class_b: int, cfg: ExperimentConfig) -> None:
    ...
    if is_pair_done(PAIR_DIR):
        print(f"SKIP {PAIR_NAME}: results.json already exists in {PAIR_DIR}", flush=True)
        return
    ...
    # =======================================================================
    X_full, y_full, subject_ids = load_all_subjects(cfg.dataset_root)

    print(f"\nBINARY CLASSIFICATION: Class {class_a} vs Class {class_b}  ({PAIR_NAME})",
          flush=True)
    X_full, y_full, subject_ids = filter_binary(X_full, y_full, subject_ids, class_a, class_b)
    ...
    if cfg._max_subjects is not None:                              # mode hook (§11.3)
    ...
    n_epochs = cfg.n_epochs                                        # mode hook (§11.3)
    n_classes = 2
    ...
    for subj_idx, subj_id in enumerate(unique_subjects):
        ...
        n_folds = cfg.n_folds                                     # mode hook (§11.3)
    ...
    n_saved = len([d for d in os.listdir(cfg.results_root)
                   if not d.startswith('_') and is_pair_done(os.path.join(cfg.results_root, d))])
    print(f"\nPairs saved so far in {cfg.results_root}: {n_saved}/{cfg.n_pairs}", flush=True)
