...
def is_pair_done(pair_dir):
    """A pair is complete once results.json exists (it is written last)."""
    import os
    return os.path.exists(os.path.join(pair_dir, 'results.json'))
...
def write_pair_results(pair_dir, pair_name, class_a, class_b, windows_ms,
    ...
    and finally results.json (atomically, so a killed job never leaves a
    half-written completion marker). `meta` carries seed / gpu / torch / model /
    mode into the payload. Returns the wide subjects x windows DataFrame.
    """
