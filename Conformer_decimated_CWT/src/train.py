...
Ported from Conformer_0.5_3hz/src/train.py. The same loop trains all three models,
so Conformer A / B / B + CWT see identical CV splits, normalization, augmentation,
seeds and epochs. Changes vs. Conformer_0.5_3hz:
  - data is decimated 250 -> 50 Hz (D = 5) at load time; windows are in 50 Hz
    samples (800 / 1500 / 3000 / 4000 ms -> 40 / 75 / 150 / 200)
  - conformer_b_cwt: after z-score + augmentation, every split goes through the
...
from src.models.conformer_b_cwt import FineConformerHybridCWT
...
def build_model(cfg: ExperimentConfig, n_channels: int, n_timepoints: int, n_classes: int = 2):
    """Dispatch on cfg.model. Each model gets its hyperparameter dict plus the
    window/sampling info it needs to size the positional embedding."""
    if cfg.model not in ('conformer_a', 'conformer_b', 'conformer_b_cwt'):
    ...
    if cfg.model == 'conformer_b':
        return FineConformerHybrid(n_channels=n_channels, n_timepoints=n_timepoints,
                                   n_classes=n_classes, embedding_dim=None, cfg=model_cfg)
    return FineConformerHybridCWT(n_channels=n_channels, n_timepoints=n_timepoints,
                                  n_freqs=cfg.cwt['n_freqs'],
                                  n_classes=n_classes, embedding_dim=None, cfg=model_cfg)


def apply_frontend(cfg: ExperimentConfig, X_train, X_val, X_test, device):
    """z-scored (+ augmented) EEG -> model input. Identity for conformer_a / _b;
    Morlet log-scalogram + per-(channel, frequency) z-score for conformer_b_cwt."""
