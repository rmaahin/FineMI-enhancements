"""Option B + CWT — Conformer B with a Morlet-scalogram front-end, on 50 Hz input.

Input is the (C, F, T) = (62, 24, T) log-magnitude scalogram from src/cwt.py. The
front-end lifts Option B's first two blocks to 2D, the way CWT-modified lifted FINE:
  ...
  freq collapse Conv2d (F, 1) + BN + ReLU over all 24 rows (the scalogram analogue
                of Conformer A's full-scalp spatial conv)          -> (B, 64, T)

Everything after that is Option B's TransformerTail, unchanged: pool 3/3 ->
positional embedding -> 2-layer encoder -> DS-conv + ECA -> head. So Conformer B
and Conformer B + CWT differ only in the front-end.
"""
from __future__ import annotations   # `int | None` annotations on Python < 3.10

import torch.nn as nn

from src.models.conformer_b import TransformerTail
...
def _default_cfg() -> dict:
    ...
    d = ExperimentConfig(model='conformer_b_cwt', dataset_root='')
    return dict(d.conformer_b_cwt,
                test_time_windows_ms=d.test_time_windows_ms,
                sampling_rate=d.sampling_rate)


class FineConformerHybridCWT(nn.Module):
    """cfg: the `conformer_b_cwt` hyperparameter dict, plus `test_time_windows_ms` and
    `sampling_rate`. Missing keys fall back to the ExperimentConfig defaults.
    """
