"""Option A — pure EEG Conformer (Song et al. 2022), adapted to FineMI, on 50 Hz input.

Same module as Conformer_0.5_3hz/src/models/conformer_a.py. At 50 Hz the config
(src/config.py) keeps the 25-sample temporal kernel (now 500 ms) and divides the
pool by D = 5 (75/15 -> 15/3), so each window still gives 9 / 21 / 46 / 62 tokens.
"""
...
def _default_cfg() -> dict:
    ...
    d = ExperimentConfig(model='conformer_a', dataset_root='')
