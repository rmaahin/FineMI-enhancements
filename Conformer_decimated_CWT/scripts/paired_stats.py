...
#
# Run from Conformer_decimated_CWT/ (submit_ls6.slurm runs it after the comparison table):
#
#   python -m scripts.paired_stats                  # every pair all models completed
#   python -m scripts.paired_stats --pairs all --mode full
#
# Contrasts: Conformer A - Conformer B, B+CWT - Conformer B, B+CWT - Conformer A
# (only those whose models are selected), for every window and the window average:
#   scope=pair    one test per pair (across subjects)
#   scope=pooled  each subject's accuracy averaged over all complete pairs first
...
CONTRASTS = [('conformer_a', 'conformer_b'), ('conformer_b_cwt', 'conformer_b'),
             ('conformer_b_cwt', 'conformer_a')]
SHORT = {'conformer_a': 'A', 'conformer_b': 'B', 'conformer_b_cwt': 'B+CWT'}
...
def main():
    ap = argparse.ArgumentParser(description="Paired tests between Conformer A / B / B + CWT.")
    ...
    if not contrasts:
        ap.error("need at least two of conformer_a / conformer_b / conformer_b_cwt in --models")
    try:
    ...
    lines = [
        ...
        "",
        f"Differences in percentage points (first minus second architecture), across "
        f"{int(df['n_subjects'].max())} subjects. "
