...
# no torch). run_selected_pairs / submit_ls6.slurm already do this at the end; use
# this to refresh the table after partial runs or to compare a different subset.
#
# Run from Conformer_decimated_CWT/:
#
#   python -m scripts.make_comparison_table                      # every completed pair
#   python -m scripts.make_comparison_table --pairs all
#   python -m scripts.make_comparison_table --pairs WAA_SAA HOC_WFE EPS_SPS
#   python -m scripts.make_comparison_table --mode smoke
#
...
from src.config import MODELS, MODE_OVERRIDES, RESULTS_ROOT_DEFAULT, mode_root, parse_pairs


def main():
    p = argparse.ArgumentParser(description="Build the Conformer A / B / B + CWT comparison table.")
