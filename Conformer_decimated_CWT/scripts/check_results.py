# Verify a finished run's outputs (no training, no torch). submit_ls6.slurm runs it
# at the end of both modes; a nonzero exit marks the job as failed.
#
#   python -m scripts.check_results --mode smoke --pairs HOC_WFE WAA_SAA EPS_SPS
#   python -m scripts.check_results --mode full --pairs all --dataset-root $DATASET_ROOT
#
# For every (pair, model): results.json exists with this pipeline's stamp and the
# run's mode / folds / epochs; wide_subject_x_window.csv has every window and the
# expected number of subjects with no missing cells and accuracies in [0, 100];
# per_fold.csv has subjects x windows x folds rows. Then the comparison table (and
# paired stats when 2+ models) must exist.
...
from src.config import (DATASET_ROOT_DEFAULT, MODELS, MODE_OVERRIDES, RESULTS_ROOT_DEFAULT,
                        make_config, mode_root, pair_slug, parse_pairs)
...
def main():
    ...
    needed = ['comparison_table.md', 'comparison_long.csv', 'comparison_mean.csv']
    if len(models) > 1:
        needed += ['paired_stats.md', 'paired_stats.csv']
    for f in needed:
        if not os.path.isfile(os.path.join(comp, f)):
            problems.append(f"comparison/{f} missing")

    total = len(pairs) * len(models)
    print(f"check_results ({args.mode}): {n_ok}/{total} pair x model runs present, "
    ...
    if problems:
        ...
        return 1
    print("CHECK PASSED: every run and table is complete", flush=True)
    return 0
