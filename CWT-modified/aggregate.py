"""
Build the FINE-raw (decimated) vs FINE-CWT comparison from saved results.

    python aggregate.py
    python aggregate.py --results-root results
...
The `shallow` and `fine_paper` columns are the ORIGINAL paper's 8-30 Hz numbers
(Table III) and are kept purely as a reference point; the runs aggregated here are on
0.5-3 Hz data decimated to 50 Hz and are not directly comparable to them.
...
def report(tab, long_df, results_root):
    show = [c for c in ['pair', 'proximity', 'n_subj', 'shallow', 'fine_paper',
                        ...
                        'p', 'p_fdr'] if c in tab]
    print("\n=== PER-PAIR (accuracy %, pooled over folds; raw = decimated 0.5-3 Hz, "
          "cwt = Morlet 0.5-3 Hz) ===")
    ...
    for col, label in (('shallow', 'ShallowConvNet (8-30 Hz paper)'),
                       ('fine_paper', 'FINE (8-30 Hz paper)')):
        for arm in ('raw_acc', 'cwt_acc'):
            if arm not in tab:
                continue
            sub = tab.dropna(subset=[col, arm])
            if len(sub):
                print(f"{arm[:3]:3s} vs {label:30s}: better on "
                      f"{int((sub[arm] > sub[col]).sum())}/{len(sub)} pairs "
                      f"(mean {sub[arm].mean() - sub[col].mean():+.2f} pp) [reference only]")
...
def main():
    ...
    args = ap.parse_args()
    root = args.results_root or os.environ.get('FINE_RESULTS_ROOT') or os.path.join(
