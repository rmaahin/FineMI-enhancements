"""
FINE fine-grained MI pipeline with a switchable feature-extraction front-end.

Single source of truth for both `run_local.py` (batch sweep) and
`EMBC_cwt_morlet.ipynb` (interactive, one pair at a time).

FEATURE_MODE:
    'raw'  - 8-30 Hz EEG as the original slices it, (C, T),  FINE 1D  (baseline)
    'cwt'  - Morlet scalograms,       (C, F, T),     FINE 2D  (Technique 2)
...
DESIGN CONTRACT
---------------
The original pipeline (EMBC_deterministic-3.ipynb) is reproduced VERBATIM. FEATURE_MODE
'raw' is bit-for-bit the original method: same window slicing, same normalisation, same
augmentation draws, same CV, same model, same (final-epoch) checkpoint behaviour.

Only what the scalogram front-end genuinely requires was added:
  * morlet_scalogram() + scalogram_normalize()   - the front-end itself
  * FINEScalogram2D                             - the 1D graph lifted to 2D over (F,T)
  * mean/amax instead of AdaptiveAvgPool2d      - the 2D adaptive pools have
        nondeterministic CUDA backwards and RAISE under the determinism flags the
        original pipeline already sets
  * left reflect-padding in the CWT context     - the window starts at the epoch edge,
        so there is no earlier data for the wavelet to use

Known defects in the original are documented in place and deliberately NOT repaired;
see the checkpoint note in run_pair(). Diagnostics live behind env vars that default to
the original behaviour, so the defaults always reproduce the paper's pipeline.
...
#   (measured on HOC/SPS: cue alignment moves accuracy by -0.29 pp, p=0.84 - no effect)
...
# ===========================================================================
# MORLET CWT FRONT-END PARAMETERS
# ===========================================================================
N_FREQS = 24
FREQS = np.logspace(np.log10(8.0), np.log10(30.0), N_FREQS)
N_CYCLES = np.linspace(3.0, 7.0, N_FREQS)   # ramp; FREQS/2 would give a Gabor transform
CWT_TRUNC = 5.0                             # kernel support in sigma_t
TIME_DECIM = 4                              # 250 Hz -> 62.5 Hz
LOG_EPS = 1e-10                             # absolute floor; see morlet_scalogram contract

# CWT batch size. Peak VRAM is roughly chunk * C * F * n_fft * 12 bytes.
# Auto-scales with the card: 16 on an 8 GB laptop GPU, 64 on a 24 GB 4090/A100.
...
# Temporal kernel sizes for the 2D MTC block. The default carries the original 1D values
# over unchanged - but the time axis is decimated by TIME_DECIM, so those kernels cover
# TIME_DECIM x LONGER spans in the cwt arm than in the raw arm:
#     raw {7,15,31} @ 250 Hz    =  28 /  60 / 124 ms
#     cwt {7,15,31} @ 62.5 Hz   = 112 / 240 / 496 ms
# FINE_MTC_KERNELS_2D=3,5,9 gives 48/80/144 ms, i.e. receptive fields matched to the raw
# arm, which isolates the front-end from the receptive-field change. Kernels must be odd
...
#   1 - initial
#   2 - checkpoint "fix" (REVERTED - it changed the original pipeline's behaviour)
#   3 - original paper pipeline preserved verbatim + CWT front-end only
PIPELINE_VERSION = 3
...
CWT_K = PSI_R_NP.shape[1]
CWT_PAD = CWT_K // 2
SIGMA_T = N_CYCLES / (2 * np.pi * FREQS)

# Context window fed to the CWT. The left side uses REAL pre-cue data where available;
# the right side is reflect-padded, so no post-window sample can ever influence the output.
...
LEFT_REFLECT = CWT_PAD - LEFT_CTX           # shortfall made up by reflection (0 normally)
...
T_OUT = WINDOW_SAMPLES // TIME_DECIM
...
def morlet_scalogram(x_np, device, chunk=None):
    ...
    magnitude -> crop pad -> log -> decimate. Runs under no_grad; the front-end is a
    fixed transform, nothing here is trainable or seeded.
    ...
    output. On z-scored input the magnitudes bottom out near 1.4e-4, i.e. 1e6 x LOG_EPS.
    """
    ...
    with torch.no_grad():
        ...
        for s in range(0, x_np.shape[0], chunk):
            ...
            # reflected only if there was not enough real pre-cue context (FINE_CUE_SAMPLE=0).
            xb = torch.nn.functional.pad(xb, (LEFT_REFLECT, CWT_PAD), mode='reflect')
            ...
            mag = torch.log(mag + LOG_EPS)[..., ::TIME_DECIM]
...
def load_pair(class_a, class_b, dataset_root=None, max_subjects=None, verbose=True):
    ...
    preserved) but holds ~0.8 GB instead of 3.2 GB.
    """
    ...
    if verbose:
        ...
        print(f"  cue at context index {CWT_PAD}, "
              f"decision window = context[{WIN_SLICE.start}:{WIN_SLICE.stop}]")
...
def run_pair(class_a, class_b, mode, device, n_epochs=N_EPOCHS, max_subjects=None,
    ...
    for subj_id in unique_subjects:
        ...
        for fold_idx, (train_val_indices, test_indices) in enumerate(cv_splits):
            ...
            # (Measured on HOC/SPS: cloning the tensors so best-val selection really works
            #  COSTS 3.50 pp, p=0.0091 - each val split is only ~18 trials, so picking the
            #  max over 50 epochs selects validation-set noise.)
...
def save_results(all_subject_results, class_a, class_b, mode, gpu,
    ...
    payload = dict(pair=name, mode=mode, class_a=int(class_a), class_b=int(class_b),
                   ...
                   frontend=dict(n_freqs=N_FREQS, fmin=float(FREQS[0]),
                                 ...
                                 decim=TIME_DECIM, kernel_len=int(CWT_K),
...
def is_done(class_a, class_b, mode, results_root=None, verbose=False):
    ...
    A results directory written by older code (or with a different cue alignment)
    reports False, so a sweep recomputes it rather than silently mixing pipelines.
    """
    root = RESULTS_ROOT if results_root is None else results_root
