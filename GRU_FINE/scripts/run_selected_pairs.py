...
# Run selected joint pairs through FINE + GRU and FINE + GRU + CWT on
# the FineMI 0.5-3 Hz data decimated to 50 Hz (D=5), then build the comparison table.
#
# Run from GRU_FINE/ as a module (puts this folder on sys.path so
# `import src.*` resolves):
#
#   cd "FineMI-enhancements/GRU_FINE"
...
#   python -m scripts.run_selected_pairs --pairs WAA_SAA --models fine_gru_cwt
...
def main():
    p = argparse.ArgumentParser(
        description="Train FINE + GRU / FINE + GRU + CWT on selected pairs (0.5-3 Hz, decimated to 50 Hz).")
