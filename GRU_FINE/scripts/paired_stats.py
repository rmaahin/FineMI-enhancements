...
#
# Run from GRU_FINE/ (submit_ls6.slurm runs it after the comparison table):
#
#   python -m scripts.paired_stats                  # every pair all models completed
#   python -m scripts.paired_stats --pairs all --mode full
#
# Contrast: GRU+CWT - GRU (the front-end effect with the GRU tail), for every window
# and the window average. Contrasts against Conformer A / B / B+CWT are in
...
CONTRASTS = [('fine_gru_cwt', 'fine_gru')]
...
def main():
    ap = argparse.ArgumentParser(description="Paired tests between FINE + GRU and FINE + GRU + CWT.")
    ...
    if not contrasts:
        ap.error("need both fine_gru and fine_gru_cwt in --models")
    try:
