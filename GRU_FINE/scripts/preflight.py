...
def check_models(cfgs, device):
    ...
    for m, cfg in cfgs.items():
        for t_ms, T in zip(cfg.test_time_windows_ms, cfg.window_samples()):
            ...
            # no positional embedding here, so every parameter (the GRU included) must
            # get a finite gradient at every window
...
# ---------------------------------------------------------------------------
def main():
    ap = argparse.ArgumentParser(description="Preflight checks for the decimated FINE + GRU sweep.")
