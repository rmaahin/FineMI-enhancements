...
# Train Slim Conformer B on selected joint pairs (0.5-3 Hz data decimated to 50 Hz).
#
# Run from Slim_conformer_fine/ as a module (puts this folder on sys.path so
# `import src.*` resolves):
#
#   python -m scripts.run_selected_pairs --pairs WAA_SAA HOC_WFE EPS_SPS --mode smoke
...
#
# For the local 3-pair test, scripts/local_test.py runs this plus the preflight,
# the output check and the comparison with Conformer A and B.
#
...
#   results/<mode>/conformer_b_slim/<PAIR>/   per_subject.csv, per_fold.csv,
#                                             wide_subject_x_window.csv, results.{json,pkl}
#   results/<mode>/conformer_b_slim/config.json
#   results/<mode>/logs/run_<timestamp>.log   copy of everything printed
#
# Completed pairs are skipped, so an interrupted run can simply be restarted with the
# same command; --overwrite re-trains them.
...
def main():
    p = argparse.ArgumentParser(
        description="Train Slim Conformer B on selected pairs (0.5-3 Hz, decimated to 50 Hz).")
    p.add_argument('--pairs', required=True, nargs='+', metavar='PAIR',
                   help="pairs to run, e.g. WAA_SAA HOC_WFE EPS_SPS (or 2_6, pair index 0-27, or all)")
    ...
    p.add_argument('--overwrite', action='store_true',
                   help="re-train pairs that already have results.json")
    p.add_argument('--no-log-file', action='store_true',
                   help="don't write results/<mode>/logs/run_*.log")
    ...
    p.add_argument('--pending', action='store_true',
                   help="with --list-pairs: only list pairs without results.json for --mode "
                        "(ignored with --overwrite)")
    args = p.parse_args()
